...
async function runCloudSQLAudit() {
  try {
    ...
    console.log('Starting Cloud SQL audit...');
    ...
    try {
      ...
      errors.push({ check: 'Instance List', error: err.message });
      summary.failed++;
      summary.totalChecks++;
    }

    // Write results
    writeAuditResults('cloudsql-audit', findings, summary, errors, projectId);
    return { findings, summary, errors };
  } catch (error) {
    console.error('Error running Cloud SQL audit:', error);
    throw error;
  }
}
