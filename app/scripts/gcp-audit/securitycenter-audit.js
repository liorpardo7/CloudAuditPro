...
async function run(projectId, tokens) {
  ...
  try {
    ...
    try {
      const findingsResp = await securitycenter.projects.sources.findings.list({ parent: `${projectName}/sources/-`, filter: 'state != "INACTIVE"' });
      ...
      findings.push({
        ...
        details: activeFindings.map(f => ({ name: f.name, severity: f.severity, category: f.category, state: f.state }))
      });
      summary.totalChecks++;
    ...
    try {
      const sourcesResp = await securitycenter.projects.sources.list({ parent: projectName });
      ...
      findings.push({
        ...
        details: vulnerabilitySources.map(s => ({ name: s.displayName, description: s.description }))
      });
      summary.totalChecks++;
    ...
    try {
      const assetsResp = await securitycenter.projects.assets.list({ parent: projectName });
      ...
      findings.push({
        ...
        details: assets.map(a => ({ name: a.name, securityCenterProperties: a.securityCenterProperties }))
      });
      summary.totalChecks++;
    ...
    try {
      const threatFindingsResp = await securitycenter.projects.sources.findings.list({ parent: `${projectName}/sources/-`, filter: 'category="THREAT" AND state != "INACTIVE"' });
      ...
      findings.push({
        ...
        details: threatFindings.map(f => ({ name: f.name, severity: f.severity, category: f.category, state: f.state }))
      });
      summary.totalChecks++;
    ...
    try {
      const recommendationsResp = await securitycenter.projects.sources.findings.list({ parent: `${projectName}/sources/-`, filter: 'category="SECURITY_RECOMMENDATION" AND state != "INACTIVE"' });
      ...
      findings.push({
        ...
        details: recommendations.map(r => ({ name: r.name, severity: r.severity, category: r.category, state: r.state }))
      });
      summary.totalChecks++;
    ...
    try {
      const monitoringResp = await securitycenter.projects.sources.findings.list({ parent: `${projectName}/sources/-`, filter: 'category="SECURITY_MONITORING" AND state != "INACTIVE"' });
      ...
      findings.push({
        ...
        details: monitoringFindings.map(f => ({ name: f.name, severity: f.severity, category: f.category, state: f.state }))
      });
      summary.totalChecks++;
    ...
    try {
      const automationResp = await securitycenter.projects.sources.findings.list({ parent: `${projectName}/sources/-`, filter: 'category="SECURITY_AUTOMATION" AND state != "INACTIVE"' });
      ...
      findings.push({
        ...
        details: automationFindings.map(f => ({ name: f.name, severity: f.severity, category: f.category, state: f.state }))
      });
      summary.totalChecks++;
    ...
    try {
      const costResp = await securitycenter.projects.sources.findings.list({ parent: `${projectName}/sources/-`, filter: 'category="COST_OPTIMIZATION" AND state != "INACTIVE"' });
      ...
      findings.push({
        ...
        details: costFindings.map(f => ({ name: f.name, severity: f.severity, category: f.category, state: f.state }))
      });
      summary.totalChecks++;
    ...
    await writeAuditResults('securitycenter-audit', findings, summary, errors, projectId);
